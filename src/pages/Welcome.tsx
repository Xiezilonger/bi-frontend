...
const blockContent = `通过我们的智能BI平台，即使对数据分析一无所知的同学也能够轻松完成复杂的数据分析任务，节约人力成本。利用AI接口和自动化技术，我们为您提供了快速、准确和可靠的数据洞察力。立即体验我们的智能BI平台，让数据分析变得简单而智能！`;
...
但是，我们的项目将改善这一现状。我们的数据分析工具不需要用户具备专业的数据分析能力，只需导入最原始的数据集并输入所需的分析目标，即可依靠强大的AI技术自动生成符合要求的图表和详细的分析结论。这一创新的方法不仅减少了数据准备和分析的时间和工作量，还使得数据分析对于非专业用户来说变得轻而易举。

此外，我们的项目还包含其他功能，如图表管理、AI智能助手和异步生成。通过图表管理功能，用户可以轻松管理已生成的图表，并可以查看原始数据。AI问答助手将回答您提出的任何问题，并且可以在对话管理界面查看往期对话内容，而异步生成功能可以让用户在图表生成的同时进行其他操作，进一步提高工作效率。

这个项目致力于让不具备数据分析技能的人员也能快速进行数据分析。我们的解决方案利用AI接口，根据用户输入的分析目标和原始数据，快速生成高质量的可视化图表和详细的分析结论，提高工作效率，并为决策提供更有力的支持。

总而言之，本项目是我们开发小队对于AIGC的一次勇敢的实践，感谢你的使用和支持。`;
    ...
    <PageContainer >
      <Card>
        ...
        <Paragraph style={{ color: 'blue' }}>
          我们的智能BI平台是一个具有AI能力的数据分析工具，为用户提供了轻松、快速且智能化的数据分析体验。
        </Paragraph>
